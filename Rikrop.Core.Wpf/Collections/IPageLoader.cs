...
namespace Rikrop.Core.Wpf.Collections
{
    ...
    {
        Task<IReadOnlyCollection<TItem>> GetPage(int skipItems, int takeItems);
    }
    ...
    namespace Contracts
    {
        ...
        public abstract class ContractIPageLoader<TItem> : IPageLoader<TItem>
        {
            public Task<IReadOnlyCollection<TItem>> GetPage(int skipItems, int takeItems)
            {
                ...
                Contract.Assume(Contract.Result<Task<IReadOnlyCollection<TItem>>>() != null);

                return default(Task<IReadOnlyCollection<TItem>>);
            }
        }
    }
}